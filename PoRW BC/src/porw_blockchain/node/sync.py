...
async def compare_with_peers():
    ...
    try:
        ...
        for peer in p2p.connected_peers:
            # Send a STATUS_REQUEST message to each peer
            message = {"type": p2p.MESSAGE_TYPES["STATUS_REQUEST"]}
            peer.writer.write(json.dumps(message).encode())
            ...
            if response.get("type") == p2p.MESSAGE_TYPES["STATUS_RESPONSE"]:
                peer_height = response.get("height", -1)
                if peer_height > best_peer_height:
                    best_peer_height = peer_height
                    target_peer = peer

        if target_peer:
            logger.info(f"Found peer {target_peer.host}:{target_peer.port} with height {best_peer_height}. Need to sync.")
...
async def request_missing_blocks(peer: 'p2p.Peer', start_index: int, end_index: int):
    """
    Requests a range of blocks from a specific peer.
    """
    logger.info(f"Requesting blocks {start_index}-{end_index} from peer {peer.host}:{peer.port}")
    try:
        message = {
            "type": p2p.MESSAGE_TYPES["BLOCK_REQUEST"],
        ...
        data = await peer.reader.read(1024)
        if not data:
            logger.warning(f"No response received from peer {peer.host}:{peer.port}")
            return

        response = json.loads(data.decode())
        if response.get("type") == p2p.MESSAGE_TYPES["BLOCK_RESPONSE"]:
            blocks = response.get("blocks", [])
            for block_data in blocks:
                await handle_received_block(block_data)

    except Exception as e:
        logger.error(f"Error requesting blocks from peer {peer.host}:{peer.port}: {e}", exc_info=True)
